use super::{
    ...
    structs::EmbeddedDeploymentTemplates,
    vars::NAMESPACE,
};
use kube::{
...
pub async fn action(
    ...
    if context.is_ok() {
        ...
        let resource_client = Api::v1Deployment(client).within(&namespace);

        for asset in EmbeddedDeploymentTemplates::iter() {
            let filename = asset.as_ref();

            // Ignore hidden files
            if !filename.starts_with(".") {
                // Create new resources
                let file_data = EmbeddedDeploymentTemplates::get(&filename).unwrap();
