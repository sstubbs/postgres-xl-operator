...
async fn create_context(
    ...
    if custom_resource.spec.data.is_some() {
        ...
        if yaml_added_object.is_ok() {
            ...
            let yaml_struct_merged_object =
                serde_yaml::from_str(&yaml_struct_merged_string_unwrapped);

            if yaml_struct_merged_object.is_ok() {
                // Create cluster object
                let yaml_struct_merged_object_unwrapped: structs::Values =
                    yaml_struct_merged_object?;
                ...
                let global_context = structs::Chart {
                    ...
                    release_name: std::env::var("RELEASE_NAME")
                        .unwrap_or(vars::RELEASE_NAME.into()),
    ...
    return Err(anyhow!("No Spec Data",));
}
