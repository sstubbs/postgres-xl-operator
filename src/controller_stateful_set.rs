use super::{
    ...
    structs::EmbeddedStatefulSetTemplates,
    vars::NAMESPACE,
};
use kube::{
...
pub async fn action(
    ...
    if context.is_ok() {
        ...
        let resource_client = Api::v1StatefulSet(client).within(&namespace);

        for asset in EmbeddedStatefulSetTemplates::iter() {
            let filename = asset.as_ref();

            // Ignore hidden files
            if !filename.starts_with(".") {
                // Create new resources
                let file_data = EmbeddedStatefulSetTemplates::get(&filename).unwrap();
